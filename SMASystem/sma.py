...
class WebConnect:
    ...
    def auth(self):
        ...
        try:
            r = requests.post(self.__url + '/dyn/login.json', headers=headers, json=params, verify=False, timeout=self.c_timeout)
        except Exception as e:
            logging.error(f"Exception during auth(): {e}")
            return False
        ...
        if r.text:
            ...
            logging.error(f"auth(): No POST response text")
            return False
    ...
    def logout(self):
        ...
        try:
            r = requests.post(self.__url + '/dyn/logout.json?sid=' + self.ssid, headers=headers, json=params,
                              verify=False, timeout=self.c_timeout)
        except Exception as e:
    ...
    def check_connection(self):
        ...
        if self.lsid is None or self.ssid is None or self.cookie is None:
            logging.error(f"check_connection: ldis={self.lsid}; ssid={self.ssid} cookie={self.cookie}")
            return False
        ...
        try:
            ...
            logging.error(f"check_connection: exception={e} url={self.__url}; headers={headers} params={params}")
            return False
        ...
        if not 'result' in json_data:
            logging.error(f"check_connection: not results in {json_data}")
            return False
        elif not 'cntDwnGg' in json_data['result']:
            logging.error(f"check_connection: not cntDwnGg in {json_data['result']}")
            return False
    ...
    def get_value(self, key: Key):
        ...
        try:
            ...
            logging.error(f"Exception during get_value(): {e}")
            return None
    ...
    def get_all_keys(self):
        ...
        try:
            ...
            logging.error(f"Exception during get_all_keys: {e}")
            return None
    ...
    def get_logger(self, start: int, end: int):
        ...
        try:
            ...
            logging.error(f"Exception during get_logger: {e}")
            return None
